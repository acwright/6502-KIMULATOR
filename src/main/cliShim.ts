 ...
 *
 * The name is `6502-kim`, not `6502`: it matches the hardware repository, and
 * it cannot collide with the `6502` shim 6502-EMULATOR installs — both
 * emulators are expected on the same machine.
 */
export class CliShimService {
  ...
  private candidatePaths(): string[] {
    return ['/usr/local/bin/6502-kim', join(homedir(), '.local', 'bin', '6502-kim')]
  }
      ...
      try {
        ...
        return {
          ...
          message: onDefaultPath
            ? `Installed at ${path}. Open a new terminal and run "6502-kim --version" to check.`
            : `Installed at ${path}. Add "${dirname(path)}" to your shell's PATH if "6502-kim" is not found — ` +
